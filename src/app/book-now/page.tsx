...
const trustPoints = [
  ...
  { icon: Star, label: "4.9 Rating", desc: "From 1,800+ guests" },
]
...
function WizardWithParams({ onLocationPin }: { onLocationPin: (lat: string, lon: string, name: string) => void }) {
  ...
  const pickupDate = searchParams.get("pickupDate") || searchParams.get("date") || ""
  const pickupTime = searchParams.get("pickupTime") || searchParams.get("time") || ""
  const deliveryDate = searchParams.get("deliveryDate") || ""
  const deliveryTime = searchParams.get("deliveryTime") || ""
  const initialDraft = { pickupLocation, dropoffLocation, pickupDate, pickupTime, deliveryDate, deliveryTime }
      ...
      >
        <div className="w-full max-w-[1400px] mx-auto flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5">
            <Image src="/Logo_primary.png" alt="Logo" width={150} height={60} priority style={{ height: '60px', width: 'auto' }} />
          </Link>
      ...
      <div className="flex flex-col lg:flex-row flex-1 pt-16 min-h-screen">
        ...
        <div className="hidden lg:flex lg:w-[42%] xl:w-[38%] relative flex-col justify-end overflow-hidden flex-shrink-0 bg-[#0A2E6D]">
          <iframe
            ...
            style={{ border: 0, top: "-80px", left: "-80px", width: "calc(100% + 160px)", height: "calc(100% + 160px)" }}
            loading="lazy"
            allowFullScreen
            src={mapSrc}
          />
          ...
          <div className="relative z-20 p-12 xl:p-16 flex flex-col gap-8">
            <AnimatePresence mode="wait">
              {pinned ? (
                <motion.div key="pinned" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  className="inline-flex items-center gap-2 self-start bg-[#1E5BD7]/20 border border-[#1E5BD7]/30 px-4 py-2 rounded-lg">
                  <span className="w-2 h-2 rounded-full bg-[#1E5BD7] animate-pulse" />
                  <span className="text-white text-sm font-semibold">{pinned.name}</span>
                </motion.div>
              ) : (
                <motion.div key="default" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  ...
                  <span className="text-white/60 text-sm font-semibold tracking-wide uppercase">Dubai, UAE</span>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <div>
              <h1 className="text-4xl xl:text-5xl font-bold tracking-tight leading-[1.1] text-white mb-4">
                Your bags.<br /><span className="text-white/40">Our mission.</span>
              </h1>
              <p className="text-white/50 text-base font-medium max-w-xs leading-relaxed">
                Hands-free travel across Dubai and the UAE. Your belongings arrive before you do.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2 border-t border-white/10">
              <div className="flex -space-x-2">
                {[41, 42, 43, 44].map(i => (
                  <Image key={i} src={`https://i.pravatar.cc/100?img=${i}`} width={28} height={28} className="w-7 h-7 rounded-full border-2 border-[#0A2E6D] object-cover" alt="" />
                ))}
              </div>
              <p className="text-white/40 text-xs font-medium">
                Trusted by <span className="text-white font-semibold">2,500+</span> travellers
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center px-6 sm:px-10 lg:px-14 xl:px-18 py-10 lg:py-12 bg-white lg:border-l border-[#E5E5E5] overflow-y-auto">
          <div className="w-full max-w-2xl mx-auto">
            <Suspense fallback={<div className="w-full h-[600px] rounded-xl bg-[#F6F2EA] animate-pulse" />}>
